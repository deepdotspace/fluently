...
import type { CardMap, DeckMap, MediaRecord, RecordEnvelope, Settings, StoredCard, StoredDeck, ThemeMap } from '../../types';
...
/** Form state emitted by `DeckEditorModal` on save. */
...
function DecksList({ decks, deckMutations, cards, cardMutations, mediaRecords, mediaMutations, themes, onDeckSelect, settings }: DecksListProps) {
  const currentTheme = useTheme();
  const isMobile = useIsMobile();

  const [deckEditorModal, setDeckEditorModal] = useState<{ isOpen: boolean; deck: StoredDeck | null }>({ isOpen: false, deck: null });
  const [deleteConfirm, setDeleteConfirm] = useState<{ isOpen: boolean; deckId: string | null }>({ isOpen: false, deckId: null });


  const decksList = useMemo(() => Object.values(decks || {}), [decks]);

  const handleSaveDeck = useCallback((formData: DeckFormData) => {
  ...
  const getDeckCardCounts = (deckId: string) => {
          ...
          {decksList.map((deck, index) => {
            ...
            return (
              <div
                ...
                className="deck-card"
                style={{
                  // Solid translucent background instead of backdrop-blur:
                  // many deck cards render in this grid at once and stacked
                  // blur layers are expensive to composite.
              ...
              >
                  ...
                  <div style={{
                    ...
                    <div
                      ...
                      onMouseLeave={(e: MouseEvent<HTMLDivElement>) => e.currentTarget.style.opacity = '0.6'}
                    >
                      <button
