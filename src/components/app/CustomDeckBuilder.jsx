...
function CustomDeckBuilder({ theme, onGenerate, onClose, availableLanguages, decks }) {
  ...
  const [selectedExistingDeck, setSelectedExistingDeck] = useState(null);
  ...
  const validate = () => {
    ...
    if (!cardCount || cardCount < 10 || cardCount > 1000) {
            ...
            <div>
              ...
              <input
                ...
                onFocus={(e) => {
                  e.stopPropagation();
                  e.target.style.borderColor = theme.primary;
                  e.target.style.boxShadow = `0 0 0 3px ${hexToRgba(theme.primary, 0.2)}`;
                }}
                onBlur={(e) => {
                  e.target.style.borderColor = errors.cardCount ? '#ef4444' : theme.cardBorder;
