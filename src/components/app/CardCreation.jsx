...
function CardCreation({ decks, cards, cardMutations, deckMutations, selectedDeck, setSelectedDeck, themes, cardTypes = {}, updateCardTypes, mediaRecords, mediaMutations }) {
  ...
  const [bulkCardTypes, setBulkCardTypes] = useState(['Basic']);
  const [generateImages, setGenerateImages] = useState(false);
  const [generateAudio, setGenerateAudio] = useState(false);
  const [errorMessage, setErrorMessage] = useState('');
  const [successMessage, setSuccessMessage] = useState('');
  const [mediaNotification, setMediaNotification] = useState(null);
  const [frontMedia, setFrontMedia] = useState({ images: [], audio: [] });
  const [backMedia, setBackMedia] = useState({ images: [], audio: [] });
  ...
  const [importedVocabJson, setImportedVocabJson] = useState(null); // Store imported JSON data for regeneration
  ...
  const validateCardMedia = useCallback((card) => {
  ...
  const generateImageForCard = useCallback(async (imagePrompt, card) => {
    try {
      ...
      if (response.success && response.data?.imageUrls?.length > 0) return response.data.imageUrls[0];
  ...
  const generateAudioForCard = useCallback(async (audioPrompt, card) => {
    try {
      ...
      const appropriateVoice = PROMPTS.getAudioVoiceForLanguage(currentDeck?.targetLang);
      ...
      if (audioResponse.success && audioResponse.data?.audioUrl) return audioResponse.data.audioUrl;
      throw new Error(audioResponse.error || 'Audio generation failed');
    } catch {
      return null;
    }
  }, [currentDeck?.targetLang]);

  const processMediaForGeneratedCards = useCallback(async (cards, mediaOptions) => {
  ...
  const processTextForStorage = (text, existingMedia) => ({
    processedText: text || '',
  ...
  const handleCreateCard = (cardData = {}) => {
    ...
    if (cardData.cardTypeId && cardData.fields) {
      ...
      if (cardType?.reversible) {
        ...
        const reversedFields = { ...fields };
        const fieldNames = cardType.fields || [];
        if (fieldNames.length >= 2) {
          const firstField = fieldNames[0];
          const secondField = fieldNames[1];
        ...
        // Cloze deletion card
        const textField = cardType.fields?.[0] || 'Text';
    ...
    const processedCustomFieldMedia = {};
    ...
    if (cardType === 'Cloze') {
      ...
      const cardId = 'card-' + generateId();
      newCards[cardId] = { id: cardId, ...baseCardData, type: 'Cloze', content: { ...baseCardData.content, text: frontProcessed.processedText, clozes } };
    } else if (cardType === 'BasicReversed') {
      const normalCardId = 'card-' + generateId();
      const reversedCardId = 'card-' + generateId();
      newCards[normalCardId] = { id: normalCardId, ...baseCardData, type: 'Basic', content: { ...baseCardData.content, front: frontProcessed.processedText, back: backProcessed.processedText } };
      newCards[reversedCardId] = { id: reversedCardId, ...baseCardData, type: 'Basic', content: { ...baseCardData.content, front: backProcessed.processedText, back: frontProcessed.processedText } };
    } else {
      const cardId = 'card-' + generateId();
      newCards[cardId] = { id: cardId, ...baseCardData, type: cardType, content: { ...baseCardData.content, front: frontProcessed.processedText, back: backProcessed.processedText } };
    }
  ...
  const handleFrontMediaGenerated = useCallback((mediaData) => {
  ...
  const handleBackMediaGenerated = useCallback((mediaData) => {
    if (mediaData.type === 'image') {
  ...
  const handleFrontMediaRemove = useCallback((type, index) => {
    if (type === 'image') {
  ...
  const handleBackMediaRemove = useCallback((type, index) => {
    if (type === 'image') {
  ...
  const handleGenerateBulk = useCallback(async (mode = activeMode) => {
    setErrorMessage('');
    setSuccessMessage('');
    ...
    try {
      ...
      console.log(`🚀 Generating cards (mode: ${mode}):`, {
        mode,
        cardCount,
        textLength: textToUse.length,
        textPreview: textToUse.substring(0, 200) + (textToUse.length > 200 ? '...' : ''),
        activeTypes
      });
      ...
      console.log('📝 AI text received:', text.substring(0, 200) + (text.length > 200 ? '...' : ''));
      ...
      console.log('📊 Parsed cards array:', {
      ...
      const limited = parsed.slice(0, cardCount);
      const processedCards = limited.map((card, idx) => {
      ...
      setErrorMessage(error.message || 'Failed to generate cards. Please try again.');
      setTimeout(() => setErrorMessage(''), 2000);
    } finally {
  ...
  const persistMediaUrl = useCallback(async (url, type) => {
    if (!url || typeof url !== 'string') return url;
    if (isMediaReference(url)) return url;
  ...
  const persistMediaObject = useCallback(async (mediaObj) => {
  ...
  const isVocabJsonFormat = useCallback((jsonData) => {
  ...
  const convertVocabJsonToCards = useCallback((jsonData, limit = null, direction = 'target-to-native') => {
    ...
    const buildBackContent = (translation, entry) => {
  ...
  const handleToolbarImageUpload = useCallback(() => {
    ...
    input.onchange = (e) => {
      ...
      reader.onload = (event) => {
        ...
        if (isFront) {
          handleFrontMediaGenerated({ type: 'image', url: event.target.result });
          ...
          handleBackMediaGenerated({ type: 'image', url: event.target.result });
  ...
  const handleToolbarAudioUpload = useCallback(() => {
    ...
    input.onchange = (e) => {
      ...
      reader.onload = (event) => {
        ...
        if (isFront) {
          handleFrontMediaGenerated({ type: 'audio', url: event.target.result });
          if (frontEditorRef.current) {
            frontEditorRef.current.insertText(placeholder);
          } else {
            setFrontText(prev => prev + placeholder);
          ...
          handleBackMediaGenerated({ type: 'audio', url: event.target.result });
          if (backEditorRef.current) {
            backEditorRef.current.insertText(placeholder);
          } else {
            setBackText(prev => prev + placeholder);
  ...
  const handleCardTypeSave = useCallback((cardTypeData) => {
    updateCardTypes({
      ...cardTypes,
      [cardTypeData.id]: cardTypeData
    });
  ...
  const handleEditCardType = useCallback((cardType) => {
