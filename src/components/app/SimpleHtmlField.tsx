...
/** A toast notification surfaced after AI generation. */
interface ToastState {
  message: string;
  type: 'error' | 'info' | 'success';
}

/** Classification result returned by the AI router prompt. */
...
/** `data` payloads returned by `miyagiAPI.post` for the endpoints used here. */
interface TextPayload { text?: string }
interface ImagePayload { imageUrls?: string[]; imageUrl?: string; url?: string }
interface AudioPayload { audioUrl?: string; url?: string }

interface SimpleHtmlFieldProps {
  value?: string;
  onChange: (html: string) => void;
  placeholder?: string;
  theme: SoftTheme;
  onMediaGenerated?: (media: { type: string; url: string }) => void;
  media?: Media;
  onMediaRemove?: (type: 'image' | 'audio', index: number) => void;
  allowCloze?: boolean;
  editorId?: string;
  // AI features
  deck?: Deck | null;
...
function SimpleHtmlField({
  ...
  const insertPlaceholderIntoEditor = useCallback((placeholderText: string) => {
    if (isHtmlMode && textareaRef.current) {
      const start = textareaRef.current.selectionStart;
      const text = textareaRef.current.value;
  ...
  const handleGenerateWithPrompt = useCallback(async (prompt: string) => {
    ...
    try {
      ...
      const classifyData = classifyResponse.data as TextPayload;

      // Check if response text is empty
      if (!classifyData?.text || classifyData.text.trim() === '') {
      ...
      const rawResponse = classifyData.text.trim();
      ...
      if (type === 'text') {
        ...
        const textData = response.data as TextPayload;
        ...
        const generatedContent = textData.text.trim();
        ...
        const imageData = imageResponse.data as ImagePayload;
        ...
        if (extractedWord) {
          ...
          const audioTextData = audioText.data as TextPayload;

          // Check if response text is empty and handle fallback
          if (!audioTextData?.text || audioTextData.text.trim() === '') {
            ...
            audioContent = audioTextData.text.trim();
          }
        }
        ...
        const audioData = audioResponse.data as AudioPayload;
        ...
        const fallbackData = response.data as TextPayload;
        ...
        const generatedContent = fallbackData.text.trim();
        const newValue = value ? value + '\n\n' + generatedContent : generatedContent;
      ...
      showToast((error as Error).message || 'Generation failed', 'error');
...
function ToolbarButton({ onClick, title, icon, theme, active = false, disabled = false, isAI = false }: ToolbarButtonProps) {
  const iconElement = typeof icon === 'string' ? icon : React.cloneElement(icon as React.ReactElement<{ color?: string }>, { color: active ? 'white' : (theme?.textPrimary || '#1f2937') });
