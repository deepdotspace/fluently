...
/** A card in the deck being previewed (pre-import shape with a fields map). */
...
/**
 * The deck being previewed. This is a pre-import / catalog deck whose shape is
 * richer than the persisted `Deck`, carrying preview-only display metadata.
 */
...
interface DeckPreviewModalProps {
  ...
  onClose: () => void;
  onImport: (targetDeckId: string | null) => void;
  theme: SoftTheme;
  decks?: DeckMap;
  initialImportMode?: string;
  initialSelectedDeck?: string | null;
}

function DeckPreviewModal({ deck, cards, onClose, onImport, theme, decks, initialImportMode, initialSelectedDeck }: DeckPreviewModalProps) {
  const [showAllWords, setShowAllWords] = useState(false);
  const [importMode, setImportMode] = useState(initialImportMode || 'new'); // 'new' or 'existing'
  const [selectedExistingDeck, setSelectedExistingDeck] = useState<string | null>(initialSelectedDeck || null);
