...
import type { ManagerField } from './FieldManager';
import RichTextRenderer from './RichTextRenderer';
import GlassCard from './GlassCard';
import LucideIcon from './LucideIcon';
import type { CardType, CardTypeMap, SoftTheme } from '../../types';

/** Per-field inline styling carried on a managed field. */
...
/** The shape `onSave` receives: a card type assembled from the editor's state. */
interface CardTypeDraft {
  id: string;
  name: string;
  fields: Array<{
    name: string;
    side: ManagerField['side'];
    order: number;
    required: boolean;
    style: FieldStyle | null;
  }>;
  fieldMetadata: CardType['fieldMetadata'] | null;
...
/** Reversible/cloze pair emitted by {@link CardTypeOptionSelector}. */
...
interface CardTypeEditorProps {
  ...
  onSave: (cardType: CardTypeDraft) => void;
  onClose: () => void;
  theme: SoftTheme | null | undefined;
}
...
function CardTypeEditor({
  ...
  // Fields
  const initialFields = useMemo<ManagerField[]>(() => {
    if (cardType?.fields) {
      let normalized: ManagerField[] = normalizeFields(cardType.fields);
      ...
      if (cardType.fieldMetadata) {
        return normalized.map(field => {
          const fieldName = getFieldName(field);
          // Legacy metadata may carry per-field `side`/`order` beyond the
          // declared `{ required }` shape; read them defensively.
          const metadata = cardType.fieldMetadata[fieldName] as
            | { required?: boolean; side?: ManagerField['side']; order?: number }
            | undefined;
          if (metadata) {
            return {
              ...field,
  ...
  const [fields, setFields] = useState<ManagerField[]>(initialFields);
  ...
  const handleStyleChange = (originalIndex: number, style: FieldStyle) => {
  ...
  const handleAIGenerate = async (side: 'front' | 'back') => {
    ...
    try {
      ...
      if (response.success && (response.data as { text?: string }).text) {
        // Clean up markdown code blocks if present
        let newHtml = (response.data as { text: string }).text.trim();
  ...
  useEffect(() => {
    if (autoGenerateTemplates && fields.length > 0) {
      // `fields` may include unplaced ('available') items and null styles;
      // the generator ignores non front/back/both fields and treats a missing
      // style as empty, so the runtime shape is compatible.
      const templates = generateTemplateFromFields(
        fields as Parameters<typeof generateTemplateFromFields>[0]
      );
  ...
  const handleCardTypeChange = useCallback((type: CardTypeOption) => {
  ...
  const handleSave = useCallback(() => {
    ...
    const cardTypeData: CardTypeDraft = {
    ...
    // `validateCardType` only inspects id/name/fields-length/templates, all of
    // which `cardTypeData` satisfies; cast over the richer draft field shape.
    const validation = validateCardType(
      cardTypeData as Parameters<typeof validateCardType>[0]
    );
    if (!validation.valid) {
 ...
 */
function PreviewSide({ side, content, theme, isFlipped }: PreviewSideProps) {
  return (
    <div
      style={{
        position: 'absolute',
