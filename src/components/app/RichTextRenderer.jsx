 ...
 */
const normalizeMediaUrl = (url, type = 'image', mediaRecords = null) => {
 ...
 */
const FlashcardImage = ({ url, alt, theme, mediaRecords }) => {
 ...
 */
const FlashcardAudio = ({ url, theme, mediaRecords }) => {
 ...
 */
const RichTextRenderer = ({ html: content, media, theme, mediaRecords }) => {
      ...
      const renderNode = (node, index) => {
        ...
        if (node.nodeType === 1) {
              ...
              if (media?.images?.[imageIndex]) {
                return (
                  <FlashcardImage
                    ...
                    alt={node.getAttribute('alt') || `Image ${imageIndex + 1}`}
                    theme={theme}
          ...
          const children = isVoid
            ? undefined
            : Array.from(node.childNodes).map((child, i) => renderNode(child, `${index}-${i}`));
 ...
 */
const renderMarkdownSpan = (text, media, baseKey, theme, mediaRecords) => {
  if (!text) return null;
 ...
 */
const renderTextWithPlaceholders = (text, media, baseKey, theme, mediaRecords) => {
  if (!text) return null;
