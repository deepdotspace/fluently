...
/**
 * Imperative API exposed by `RobotViewer` via `forwardRef`. `ReviewMode`
 * consumes this ref to trigger named animations from the rating buttons.
 */
export interface RobotViewerHandle {
  /** Play the named GLTF animation clip (no-op if unavailable). */
...
const RobotViewer = React.forwardRef<RobotViewerHandle, RobotViewerProps>(function RobotViewer(
  { modelUrl = ROBOT_GLB_URL, onClick },
  ref
) {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  // The Three.js scene graph objects are untyped CDN globals; see globals-three.d.ts.
  ...
  const modelBoundsRef = useRef<{ radius: number; height: number; centerY: number } | null>(null); // store model bounds for responsive camera
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const animationsMapRef = useRef<Map<string, any>>(new Map());
  ...
  useEffect(() => {
    ...
    const waitForGlobal = (varName: string, timeout = 5000): Promise<unknown> => {
      return new Promise((resolve, reject) => {
        const startTime = Date.now();
        const checkInterval = setInterval(() => {
          if ((window as unknown as Record<string, unknown>)[varName]) {
            clearInterval(checkInterval);
            resolve((window as unknown as Record<string, unknown>)[varName]);
    ...
    const loadThreeJS = async () => {
      ...
      try {
        ...
        throw new Error('Failed to load 3D library: ' + (err as Error).message);
      }
    };
    ...
    const initScene = async () => {
      try {
        ...
        setError((err as Error).message || 'Failed to initialize 3D viewer');
        setLoading(false);
      }
    };
