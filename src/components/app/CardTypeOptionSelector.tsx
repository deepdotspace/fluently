...
/** The reversible/cloze pair this selector reads and emits. */
interface CardTypeOption {
  reversible: boolean;
  isCloze: boolean;
}

/** One of the three visual card-type choices rendered as a button. */
interface CardTypeChoice {
  id: string;
  icon: string;
  name: string;
  description: string;
...
interface CardTypeOptionSelectorProps {
  value?: Partial<CardTypeOption>;
 ...
 */
function CardTypeOptionSelector({ value, onChange, theme }: CardTypeOptionSelectorProps) {
  const cardTypes: CardTypeChoice[] = [
    {
      id: 'basic',
      icon: 'Book',
      name: 'Basic',
  ...
  const handleSelect = (type: CardTypeChoice) => {
    onChange({
