...
const adjustPlaceholders = (text = '', type: string, removedIndex: number) => text.replace(new RegExp(`\\[${type}:(\\d+)\\]`, 'g'), (match, num) => {
...
/** Per-side media for a generated card: ordered URL / `media:` reference arrays. */
export interface GeneratedCardMedia {
  images: string[];
  audio: string[];
}

/**
 * A card held in the in-memory "generated cards" working array before save.
 * Carries both the legacy (front/back) and new (cardTypeId/fields) shapes,
 * plus AI media prompts and per-side media collected during generation.
 */
export interface GeneratedCard {
  ...
  back?: string;
  tags?: string[];
  frontMedia?: GeneratedCardMedia;
  backMedia?: GeneratedCardMedia;
  imagePrompt?: string;
  audioPrompt?: string;
  cardTypeId?: string;
...
/** Notification surfaced after media (re)generation. */
...
interface GeneratedCardsPanelProps {
...
function GeneratedCardsPanel({
  ...
  const [expandedFrontMedia, setExpandedFrontMedia] = useState<GeneratedCardMedia>({ images: [], audio: [] });
  const [expandedBackMedia, setExpandedBackMedia] = useState<GeneratedCardMedia>({ images: [], audio: [] });
  const [regeneratingMedia, setRegeneratingMedia] = useState<RegeneratingMedia | null>(null);

  const handleExpandCard = useCallback((idx: number) => {
  ...
  const removeGeneratedCard = useCallback((id: string) => {
  ...
  const handleRegenerateImage = useCallback(async (side: 'front' | 'back', index: number) => {
    if (expandedCardIndex === null) return;
  ...
  const handleRegenerateAudio = useCallback(async (side: 'front' | 'back', index: number) => {
    if (expandedCardIndex === null) return;
  ...
  const mediaButton = (label: string, onClick: () => void, disabled: boolean) => (
    <button
      onClick={onClick}
      disabled={disabled}
      style={{
  ...
  const renderMediaList = useCallback((items: string[], side: 'front' | 'back', type: 'image' | 'audio') => {
