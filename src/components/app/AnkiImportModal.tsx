...
import type { CardTypeMap, DeckMap, MediaRecord, RecordEnvelope, Settings, SoftTheme } from '../../types';
...
/** Summary statistics shown after parsing / importing a package. */
...
export default function AnkiImportModal({
  ...
  const [parsedData, setParsedData] = useState<Awaited<ReturnType<typeof parseApkgFile>> | null>(null);
  ...
  const handleFileChange = useCallback(async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setError((err as Error)?.message || 'Failed to parse .apkg file.');
      setSummary(null);
    } finally {
      setParsing(false);
    }
  ...
  const handleImport = useCallback(async () => {
    ...
    try {
      ...
      if (result.cardTypesToAdd && updateCardTypes) {
        // cardTypesToAdd entries are MappedCardType (importer-local shape);
        // they satisfy the persisted CardType contract at runtime.
        updateCardTypes({
          ...(cardTypes || {}),
      ...
      setError((err as Error)?.message || 'Failed to import Anki deck.');
    } finally {
      setImporting(false);
    }
      ...
      >
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '24px' }}>
          ...
          <button
            ...
            onMouseEnter={(e: MouseEvent<HTMLButtonElement>) => (e.target as HTMLElement).style.background = 'rgba(0,0,0,0.1)'}
            onMouseLeave={(e: MouseEvent<HTMLButtonElement>) => (e.target as HTMLElement).style.background = 'rgba(0,0,0,0.05)'}
          >
        ...
        <div
          ...
          onMouseEnter={(e: MouseEvent<HTMLDivElement>) => e.currentTarget.style.borderColor = theme?.primary as string}
          onMouseLeave={(e: MouseEvent<HTMLDivElement>) => e.currentTarget.style.borderColor = theme?.cardBorder as string}
        >
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px' }}>
          <button
            ...
            onMouseEnter={(e: MouseEvent<HTMLButtonElement>) => (e.target as HTMLElement).style.background = 'rgba(0,0,0,0.03)'}
            onMouseLeave={(e: MouseEvent<HTMLButtonElement>) => (e.target as HTMLElement).style.background = 'transparent'}
            disabled={importing}
          >
            Cancel
          </button>
          <button
            ...
            onMouseEnter={(e: MouseEvent<HTMLButtonElement>) => {
              if (parsedData && !importing) {
                ...
                (e.target as HTMLElement).style.boxShadow = `0 15px 20px -3px ${theme?.primary}60`;
              }
            }}
            onMouseLeave={(e: MouseEvent<HTMLButtonElement>) => {
              (e.target as HTMLElement).style.transform = 'translateY(0)';
              (e.target as HTMLElement).style.boxShadow = parsedData && !importing ? `0 10px 15px -3px ${theme?.primary}40` : 'none';
            }}
          >
            {importing ? (
