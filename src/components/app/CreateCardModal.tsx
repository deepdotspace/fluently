...
import type { Deck, DeckMap, SoftTheme } from '../../types';
...
/** Per-field media buckets attached to a generated card. */
interface FieldMediaBucket {
  images: string[];
  audio: string[];
}

/** A card built by this modal (shape consumed by `onCreateCards`). */
interface BuiltCard {
  id: string;
  deckId: string;
  cardTypeId: string;
  type: string;
  content: {
    // `Word`/`Translation` can be undefined when the source VocabEntry lacks
    // the field; preserved faithfully rather than coerced to '' at this layer.
...
type CardDirection = 'target-to-native' | 'native-to-target';
...
interface CreateCardModalProps {
  ...
  theme: SoftTheme;
  onClose: () => void;
  onCreateCards: (cards: BuiltCard[]) => void | Promise<void>;
  // Accepted for API parity with callers but unused inside this modal.
  cardMutations?: unknown;
...
function CreateCardModal({
  ...
  const [cardDirection, setCardDirection] = useState<CardDirection>('target-to-native'); // 'target-to-native' or 'native-to-target'
  const [wordsToCreate, setWordsToCreate] = useState<VocabEntry[]>(words);
  ...
  const handleRemoveWord = useCallback((wordToRemove: VocabEntry) => {
  ...
  const fetchAudioForWord = useCallback(async (wordText: string) => {
    ...
    if (!audioData) {
      try {
        ...
        if (audioResponse.success) {
          const responseData = audioResponse.data as { audioUrl?: string } | string | undefined;
          if (responseData && typeof responseData === 'object' && responseData.audioUrl) {
            audioData = responseData.audioUrl;
  ...
  const createCardData = useCallback((word: VocabEntry, audioData: string | null, audioSource: string | null): BuiltCard => {
  ...
  const createReverseCard = useCallback((originalCard: BuiltCard, word: VocabEntry, audioData: string | null): BuiltCard => {
  ...
  const handleCreate = useCallback(async () => {
    ...
    try {
      ...
      setError((err as Error).message || 'Failed to create cards');
      setCreating(false);
    }
        ...
        >
          ...
          <div style={{ marginBottom: '20px' }}>
            ...
            <CustomDropdown
              ...
              options={Object.values(decks).map((deck: Deck) => ({ value: deck.id, label: deck.name }))}
            ...
            <div style={{
              ...
              <label style={{
                ...
                <input
                  ...
                  onChange={(e: ChangeEvent<HTMLInputElement>) => !creating && setCardDirection(e.target.value as CardDirection)}
                  disabled={creating}
          ...
          <button
            ...
            onMouseEnter={(e: MouseEvent<HTMLButtonElement>) => {
              if (!creating) {
                ...
                (e.target as HTMLElement).style.borderColor = theme.textSecondary;
              }
            }}
            onMouseLeave={(e: MouseEvent<HTMLButtonElement>) => {
              if (!creating) {
          ...
          <button
            ...
            onMouseEnter={(e: MouseEvent<HTMLButtonElement>) => {
              if (!creating && selectedDeckId && wordsToCreate.length > 0) {
                ...
                (e.target as HTMLElement).style.boxShadow = `0 6px 20px ${theme.primary}40`;
              }
            }}
            onMouseLeave={(e: MouseEvent<HTMLButtonElement>) => {
              if (!creating && selectedDeckId && wordsToCreate.length > 0) {
                (e.target as HTMLElement).style.transform = 'translateY(0)';
                (e.target as HTMLElement).style.boxShadow = `0 4px 16px ${theme.primary}30`;
              }
            }}
          >
            {creating
