 ...
 * legacy cards carry `text`/`clozes` (Cloze) or `front`/`back`. Both formats
 * are read here, so the strict `CardContent` (string map) is widened.
 */
...
/** Memoized display-text accessor built per render. */
interface DisplayTextCache {
  get: (cardId: string, side: 'front' | 'back') => string;
}
...
const getCardDisplayText = (card: LegacyCard | null | undefined, side: 'front' | 'back', allCardTypes: CardTypeMap): string => {
...
function BrowseMode({ cards, cardMutations, decks, selectedDeckId, cardTypes = {}, updateCardTypes, mediaRecords, mediaMutations, isMobile = false }: BrowseModeProps) {
  const theme = useTheme();

  // Merge default and custom/imported card types once
  const mergedCardTypes = useMemo<CardTypeMap>(() => ({
    ...getDefaultCardTypes(),
    ...(cardTypes || {})
  }), [cardTypes]);
  ...
  const filteredCards = useMemo(() => {
    ...
    if (debouncedSearch) {
      ...
      filtered = filtered.filter(card => {
        ...
        if (cardType && legacyCard.content?.fields) {
          ...
          if (legacyCard.type === 'Cloze') {
            const text = stripHTML(legacyCard.content?.text || '').toLowerCase();
            return text.includes(query);
          } else {
            const front = stripHTML(legacyCard.content?.front || '').toLowerCase();
            const back = stripHTML(legacyCard.content?.back || '').toLowerCase();
  ...
  const displayTextCache = useMemo<DisplayTextCache>(() => {
  ...
  const handleBulkTags = (action: string) => {
...
function HeaderCell({ label, sortKey, currentSort, sortOrder, onSort, theme }: HeaderCellProps) {
...
function TextCell({ text, theme }: TextCellProps) {
  const [showTooltip, setShowTooltip] = useState(false);
  const [tooltipStyle, setTooltipStyle] = useState<TooltipStyle>({ top: 0, left: 0, triangleLeft: 16 });
...
interface CardRowProps {
  ...
  onToggleSelect: () => void;
  displayTextCache?: DisplayTextCache;
  mergedCardTypes: CardTypeMap;
}

function CardRow({ card, onExpand, onDelete, onSuspend, theme, isSelected, onToggleSelect, displayTextCache, mergedCardTypes }: CardRowProps) {
...
function ActionButton({ icon, tooltip, onClick, color = '#6B7280', theme }: ActionButtonProps) {
  const [hovering, setHovering] = useState(false);
