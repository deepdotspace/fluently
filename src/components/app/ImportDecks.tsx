...
import type { VirtualDeckEntry, VirtualDeckIndex, GeneratedDeckMetadata, GeneratedVocabCard } from '../../utils/vocabDeckGenerator';
import { generateId } from '../../utils/storage';
import { saveCards, getAllCards } from '../../utils/cardStorage';
import { loadVocabFile, getVocabFileName } from '../../utils/vocabLoader';
import type { VocabDataCache } from '../../utils/vocabLoader';
import type { CardMap, CardTypeMap, DeckMap, MediaRecord, RecordEnvelope, Settings } from '../../types';
...
/**
 * Display-only deck metadata built on mount for the deck cards. This is a
 * subset of {@link GeneratedDeckMetadata} (only the fields the cards render).
 */
interface DeckCardMetadata {
  id: string;
  name: string;
  description: string;
  language: string;
...
/** Params emitted by `CustomDeckBuilder.onGenerate`. */
...
function ImportDecks({ decks, deckMutations, cardMutations, cards: allCards, onDeckSelect, cardTypes, updateCardTypes, settings, mediaRecords, mediaMutations }: ImportDecksProps) {
  const theme = useTheme();
  const isMobile = useIsMobile();
  const [index, setIndex] = useState<VirtualDeckIndex | null>(null);
  const [deckMetadata, setDeckMetadata] = useState<Record<string, DeckCardMetadata>>({});
  const [selectedLanguage, setSelectedLanguage] = useState<string | null>(null);
  const [searchQuery, setSearchQuery] = useState('');
  const [previewDeck, setPreviewDeck] = useState<GeneratedDeckMetadata | null>(null);
  const [previewCards, setPreviewCards] = useState<GeneratedVocabCard[]>([]);
  const [importing, setImporting] = useState(false);
  const [importProgress, setImportProgress] = useState<{ current: number; total: number }>({ current: 0, total: 0 });
  const [generating, setGenerating] = useState(false);
  const [generationError, setGenerationError] = useState<string | null>(null);
  const [importSuccess, setImportSuccess] = useState<string | null>(null);
  const [vocabDataCache, setVocabDataCache] = useState<VocabDataCache>({});
  const [vocabDataLoading, setVocabDataLoading] = useState(true);
  const [showCustomBuilder, setShowCustomBuilder] = useState(false);
  const [showAdvanced, setShowAdvanced] = useState(false);
  const [previewTargetDeckId, setPreviewTargetDeckId] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    if (virtualIndex && virtualIndex.decks) {
      const metadata: Record<string, DeckCardMetadata> = {};
  ...
  const filteredDecks = useMemo(() => {
    ...
    // Search
    if (searchQuery) {
      result = searchDecks(result, searchQuery, deckMetadata as unknown as Record<string, GeneratedDeckMetadata>);
    }
  ...
  const handlePreview = useCallback(async (deckEntry: VirtualDeckEntry) => {
    ...
    try {
      ...
      let vocabData: VocabEntry[] | null = vocabDataCache[fileName];
      if (!vocabData) {
      ...
      setGenerationError(`Failed to generate deck: ${(error as Error).message || 'Unknown error'}`);
    } finally {
      setGenerating(false);
    }
  }, [vocabDataCache]);
  ...
  const handleCustomGenerate = useCallback(async (params: CustomGenerateParams) => {
    ...
    try {
      ...
      let existingCards: CardMap | null = null;
      ...
      // Generate metadata
      const metadata = generateDeckMetadata(virtualDeckEntry as unknown as VirtualDeckEntry, cards);
  ...
  // Handle import
  const handleImport = useCallback(async (targetDeckId: string | null = null) => {
    ...
    try {
      ...
      previewCards.forEach((card, index) => {
        ...
        cardMutations.create({
          ...
          tags: card.tags || [],
          metadata: (card as unknown as { metadata?: Record<string, unknown> }).metadata || {},
          revLog: [],
          version: '1.0',
          createdAt: now,
          updatedAt: now,
      ...
      {showAdvanced ? (
        ...
        <>
          ...
          >
            ...
            >
              <GlassCard flat theme={theme} style={{ cursor: 'pointer', border: `2px dashed ${theme.primary}40`, height: '100%' }}>
                <div
            ...
            {filteredDecks.length === 0 ? (
              ...
              filteredDecks.map((deckEntry) => (
                <div
                  ...
                  style={{ borderRadius: '24px', cursor: 'pointer', transition: 'all 0.2s ease-out' }}
                >
                  <GlassCard flat theme={theme} style={{ cursor: 'pointer', height: '100%' }}>
                    <div
