...
/**
 * Per-field inline styling carried on a managed field and forwarded to
...
/** A field as managed here. `side` may be 'available' for unplaced fields. */
...
/** A managed field augmented with its index in the parent `fields` array. */
type IndexedField = ManagerField & { originalIndex: number };

/** A virtual "available" pool item used only for rendering/dragging. */
...
/** Drag-and-drop side identifier. */
type DragSide = 'front' | 'back' | 'available';

interface DragState {
  draggedIndex: number | null;
  draggedSide: DragSide | null;
  draggedName: string | null;
  targetIndex: number | null;
  targetSide: DragSide | null;
}

interface FieldManagerProps {
  fields: ManagerField[];
  onFieldsChange: (fields: ManagerField[]) => void;
  theme: SoftTheme;
...
function FieldManager({
  ...
  const frontFields = useMemo<IndexedField[]>(() => {
    return fields
  ...
  const backFields = useMemo<IndexedField[]>(() => {
    return fields
      .map((f, idx) => ({ ...f, originalIndex: idx }))
  ...
  const handleToggleRequired = (originalIndex: number) => {
    const updated = [...fields];
    updated[originalIndex].required = !updated[originalIndex].required;
    onFieldsChange(updated);
  };

  const handleDeleteField = (originalIndex: number) => {
    if (fields.length <= 1) {
  ...
  const handleMoveField = (originalIndex: number, direction: 'up' | 'down') => {
    const updated = [...fields];
  ...
  const handleDragStart = (index: number, side: DragSide) => {
    if (side === 'available') {
  ...
  const handleDragOver = (index: number, side: DragSide) => {
    setDragState(prev => ({
      ...prev,
      targetSide: side
    }));
  ...
  const handleDrop = (index: number, side: DragSide) => {
        ...
        <div style={{ marginBottom: '32px' }}>
          ...
          <div style={{
            ...
            <input
              type="text"
              value={newFieldName}
              onChange={(e: ChangeEvent<HTMLInputElement>) => setNewFieldName(e.target.value)}
