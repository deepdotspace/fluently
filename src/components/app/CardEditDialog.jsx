 ...
 */
function CardEditDialog({ card, cardMutations, mediaRecords, mediaMutations, decks, cardTypes = {}, theme, onClose, onSave, updateCardTypes }) {
  ...
  useEffect(() => {
    if (card.cardTypeId && card.content?.fields) {
      ...
      setFieldValues(card.content.fields || {});
      setFieldMedia(card.content.fieldMedia || {});
    } else {
      ...
      if (card.type === 'Cloze') {
        legacyFields['Text'] = card.content?.text || '';
        legacyMedia['Text'] = { images: [], audio: [] };
      } else {
        legacyFields['Front'] = card.content?.front || '';
        legacyFields['Back'] = card.content?.back || '';
        legacyMedia['Front'] = card.content?.frontMedia || { images: [], audio: [] };
        legacyMedia['Back'] = card.content?.backMedia || { images: [], audio: [] };
      }
  ...
  const handleFieldMediaGenerated = useCallback((fieldName, mediaData) => {
    setFieldMedia(prev => {
  ...
  const handleFieldMediaRemove = useCallback((fieldName, type, index) => {
    setFieldMedia(prev => {
      const fieldMediaData = prev[fieldName] || { images: [], audio: [] };
      if (type === 'image') {
        return {
    ...
    try {
      for (const [fieldName, media] of Object.entries(fieldMedia)) {
        processedFieldMedia[fieldName] = {
          ...
          audio: await Promise.all((media.audio || []).map(async (dataUrl) => {
            if (isMediaReference(dataUrl)) {
              return dataUrl;
            }
            if (typeof dataUrl === 'string' && dataUrl.startsWith('data:')) {
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: isMobile ? '16px' : '28px' }}>
                {fieldNames.map((fieldName, index) => {
                  ...
                  const isClozeField = card.type === 'Cloze' || activeCardType?.isCloze;
              ...
              <div style={{
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '12px' }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    ...
                    <span style={{ color: theme.textPrimary, fontWeight: '600' }}>{new Date(card.createdAt).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' })}</span>
                  </div>
                </div>
              </div>
            </div>)}
