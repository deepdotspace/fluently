...
import type { CardMap, DeckMap, SoftTheme, StoredCard } from '../../types';
 ...
 */
function WordLibrary({ decks, cards, cardMutations, theme }: WordLibraryProps) {
  // Language and dataset state
  const [selectedLanguage, setSelectedLanguage] = useState('Spanish');
  const [dataset, setDataset] = useState<VocabEntry[]>([]);
  const [loading, setLoading] = useState(false);
  const [loadError, setLoadError] = useState<string | null>(null);

  // Filter state
  const [selectedCefrLevels, setSelectedCefrLevels] = useState<string[]>(['A1', 'A2']);
  ...
  const [existingCardWords, setExistingCardWords] = useState<Set<string>>(new Set());

  // Modal state
  const [showCreateModal, setShowCreateModal] = useState(false);
  const [wordsToCreate, setWordsToCreate] = useState<VocabEntry[]>([]);
  ...
  const handleSelectWord = (word: VocabEntry) => {
    setSelectedWord(word);
  };

  // Handle multi-select toggle
  const handleToggleSelect = (wordId: string | undefined) => {
  ...
  // Handle create card
  const handleCreateCard = (words: VocabEntry[]) => {
  ...
  const handleCreateCards = useCallback(async (newCards: unknown[]) => {
    if (!cardMutations) return;

    for (const card of newCards) {
      try {
        // Cards built by CreateCardModal carry a fields/fieldMedia content
        // shape; saveCard/prepareCardData accept it at runtime.
        saveCard(cardMutations, card as StoredCard);
      } catch (error) {
