...
import type { Card, CardContent, Deck, ReviewRating, SoftTheme } from '../../types';

/** Loose runtime view of a pronunciation card's `content.fields`. */
type LegacyCard = Omit<Card, 'content'> & { content: { fields?: CardContent;[key: string]: unknown } };

/** AI pronunciation evaluation parsed from the `/generate-text` response. */
...
/** Narrowed `data` payload for a successful `/speech-to-text` call. */
interface SpeechToTextResult {
  text?: string;
}

interface PronunciationReviewProps {
  card: Card;
  theme: SoftTheme;
 ...
 */
function PronunciationReview({ card, theme, onRating, deck = null }: PronunciationReviewProps) {
  // Loose view for mixed-format `content` dynamic access.
  const legacyCard = card as LegacyCard | undefined;
  ...
  const [suggestedRating, setSuggestedRating] = useState<ReviewRating | null>(null);
  ...
  const rawTargetWord = legacyCard?.content?.fields?.Word || '';
  const rawTranslation = legacyCard?.content?.fields?.Translation || '';
  const rawPronunciationGuide = legacyCard?.content?.fields?.Pronunciation || '';
  const rawHint = legacyCard?.content?.fields?.Hint || '';
  ...
  const processAudio = async (blob: Blob) => {
    ...
    try {
      ...
      const transcribedText = (transcribeResponse.data as SpeechToTextResult)?.text || '';
      ...
      const evalText = (evalResponse.data as GenerateTextResult).text;

      // Parse AI response (handle both JSON and text)
      let evalData: PronunciationEvaluation;
      try {
      ...
      setError((err as Error).message || 'Failed to process audio. Please try again.');
    } finally {
  ...
  const scoreToRating = (score: number): ReviewRating => {
  ...
  const getRatingLabel = (rating: ReviewRating | null): string => {
    const labels = ['Again', 'Hard', 'Good', 'Easy'];
    return labels[(rating as number) - 1];
  };

  const getRatingColor = (rating: ReviewRating | null): string => {
    const colors = ['#EF4444', '#F59E0B', '#10B981', '#3B82F6'];
    return colors[(rating as number) - 1];
  };
  ...
  const handleCustomRating = (rating: ReviewRating) => {
    if (onRating) {
      onRating(rating);
    }
  };
          ...
          <div style={{
            ...
            <button
              ...
              onMouseEnter={(e) => {
                ...
                (e.target as HTMLElement).style.boxShadow = `0 6px 20px ${getRatingColor(suggestedRating)}50`;
              }}
              onMouseLeave={(e) => {
                (e.target as HTMLElement).style.transform = 'translateY(0)';
                (e.target as HTMLElement).style.boxShadow = `0 4px 16px ${getRatingColor(suggestedRating)}40`;
              }}
            >
            ...
            <div style={{
              ...
              {([1, 2, 3, 4] as ReviewRating[]).filter(r => r !== suggestedRating).map(rating => (
                <button
                  key={rating}
                  ...
                  onMouseEnter={(e) => {
                    (e.target as HTMLElement).style.background = `${getRatingColor(rating)}15`;
                    (e.target as HTMLElement).style.borderColor = getRatingColor(rating);
                    (e.target as HTMLElement).style.color = getRatingColor(rating);
                  }}
                  onMouseLeave={(e) => {
