...
/** Media records as returned by the SDK, passed through to `RichTextRenderer`. */
...
interface CardPreviewProps {
  ...
  media?: Media;
  deckDesignId?: string | null; // Design ID from deck
  // Legacy API: for backward compatibility
  frontText?: string;
  backText?: string;
  frontMedia?: Media;
  backMedia?: Media;
  customFields?: CardContent;
...
function CardPreview({
  ...
  const allMedia = useMemo(() => {
    if ((media?.images?.length || 0) > 0 || (media?.audio?.length || 0) > 0) {
      return media;
    }
 ...
 */
function CardSide({ side, content, media, theme, isFlipped, isEmpty, compact, mediaRecords, deckDesignId }: CardSideProps) {
  ...
  const mergedStyles = useMemo((): React.CSSProperties => {
