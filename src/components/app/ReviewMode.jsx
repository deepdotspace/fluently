...
function ReviewMode({ dueCards: _dueCards, cards, setCards, cardMutations, decks, deckMutations, settings, selectedDeckId, showSparky, cardTypes = {}, updateCardTypes, dailyProgress, updateDailyProgress, onSessionStatsChange, onBack, stats, mediaRecords, mediaMutations, isMobile = false }) {
  ...
  const triggerRobotAnimationForRating = (rating) => {
  ...
  const handleRating = (rating) => {
    ...
    // Update scheduling
    const newScheduling = calculateNextReview(currentCard, rating, settings);
    ...
    if (currentCard.scheduling.state === 'new') {
      const updatedProgress = recordNewCardSeen(dailyProgress, selectedDeckId, currentCard.id);
...
function FlashcardSide({ content, label, flipped, theme, currentCard, showHint, showScheduling, media: providedMedia, mediaRecords, deckDesignId }) {
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(() => {
      ...
      const textContent = mainDiv.textContent.trim();
      if (!textContent) return;

      // Find the parent container to get available width
      let parentContainer = mainDiv.parentElement;
      ...
      if (textWidth > availableWidth && currentFontSize > 20) {
        ...
        mainDiv.style.whiteSpace = 'nowrap'; // Prevent wrapping
      }
    }, 150);
...
function RatingButton({ label, sublabel, color, onClick, shortcut, theme, compact = false }) {
...
function StatBadge({ label, value, color, theme, compact = false }) {
  return (
    <div style={{
      display: 'flex',
      alignItems: 'baseline',
...
function ClozeCard({ card, theme, clozeAnswers, setClozeAnswers, showResult, onReveal, compact = false }) {
  // Parse the text and extract cloze positions
  const parseClozeText = () => {
