...
/** Shape of the relevant fields in a Wiktionary `action=parse` response. */
...
/** A single imageinfo record returned by the Commons API. */
interface CommonsImageInfo {
  url?: string
}

/** A single page object in a Commons `action=query` response. */
interface CommonsPage {
  missing?: string
  imageinfo?: CommonsImageInfo[]
}

/** Shape of the relevant fields in a Commons `action=query` response. */
...
/** Result returned by {@link getWiktionaryAudio}. */
export interface WiktionaryAudioResult {
  source: string
  url: string
}
 ...
 * @returns The wikitext content or null if not found
 */
 ...
 * @param wikitext - The wikitext content
 * @returns Array of audio filenames (without duplicates)
 */
function extractAudioFilenames(wikitext: string | null | undefined): string[] {
  if (!wikitext) return [];
 ...
 */
async function resolveCommonsAudioUrl(filename: string): Promise<string | null> {
  try {
 ...
 * @param language - Optional language hint for preference
 * @returns Audio object or null if not found
 */
export async function getWiktionaryAudio(
  term: string,
  language: string | null = null
): Promise<WiktionaryAudioResult | null> {
  if (!term || !term.trim()) {
    return null;
  }
