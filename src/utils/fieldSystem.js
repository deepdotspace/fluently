 ...
 * @param {Object} fields - Field values object
...
export function renderTemplate(template, fields) {
  ...
  result = result.replace(invertedRegex, (match, fieldName, content) => {
    const trimmedFieldName = fieldName.trim();
    const value = fields[trimmedFieldName];
...
export function createField(name, options = {}) {
  return {
    ...
    type: options.type || 'text', // 'text', 'image', 'audio', 'html'
    side: options.side || 'back', // 'front', 'back', 'both'
    order: options.order !== undefined ? options.order : 999
  };
}
 ...
 * @param {string[]|Object[]} fields - Array of field names or field objects
 * @returns {Object[]} - Array of normalized field objects
 */
export function normalizeFields(fields) {
  ...
  if (typeof fields[0] === 'string') {
    return fields.map((name, index) => ({
      name,
      side: index === 0 ? 'front' : 'back', // First field front, rest back (default behavior)
      order: index,
 ...
 * @param {Object[]} fields - Array of field objects with name, side, order
 * @returns {Object} - Template object with frontTemplate, backTemplate, css
 */
/**
 * Generate inline styles for a field based on its styling properties
 * @param {Object} field - Field object with style property
 * @returns {string} - Inline style attribute
 */
function generateFieldStyles(field) {
  const styles = [];
...
export function generateTemplateFromFields(fields) {
 ...
 */
export function getDefaultCardTypes() {
  return {
    'basic': {
      id: 'basic',
      name: 'Basic',
...
/**
 * Get card type by ID
 * @param {Object} cardTypes - Card types object
 * @param {string} id - Card type ID
 * @returns {Object|null} - Card type or null
 */
export function getCardType(cardTypes, id) {
  return cardTypes?.[id] || null;
}

/**
 * Get required field names from a card type
 * @param {Object} cardType - Card type object
 * @returns {string[]} - Array of required field names
 */
export function getRequiredFields(cardType) {
  if (!cardType) return [];
 ...
 * @param {Object} cardType - Card type to validate
 ...
 * @returns {string} - Processed text with cloze spans
 */
export function processCloze(text) {
  if (!text) return text;
 ...
 * @param {Object} fieldValues - Field values object
 * @param {string} frontTemplate - Front template (for {{FrontSide}})
 * @returns {string} - Rendered HTML
 */
export function renderCardTemplate(template, fieldValues, frontTemplate = null) {
  if (!template) return '';
  ...
  result = result.replace(clozeRegex, (match, fieldName) => {
 ...
 * @param {string} template - Template string
 * @param {string[]} fieldNames - Array of field names to show as placeholders
 * @param {string} frontTemplate - Front template (for {{FrontSide}})
 * @returns {string} - Rendered HTML with placeholders
 */
export function renderTemplateWithPlaceholders(template, fieldNames = [], frontTemplate = null) {
  if (!template) return '';
