...
import type { MediaRecord, MediaType, RecordEnvelope } from '../types'

/** A media record as returned by `useQuery('media')`. */
 ...
 */
export async function generateHash(data: Blob | ArrayBuffer): Promise<string> {
  try {
...
export function getExtension(type: MediaType, mimeType: string = ''): string {
  if (type === 'image') {
...
export function getMimeType(type: MediaType, ext: string): string {
  if (type === 'image') {
    ...
    const m: Record<string, string> = { mp3: 'audio/mpeg', wav: 'audio/wav', ogg: 'audio/ogg', m4a: 'audio/mp4' }
 ...
 * @returns Compressed image blob (JPEG)
 */
 ...
 */
function findMediaByMediaId(mediaRecords: MediaEnvelope[] | null | undefined, mediaId: string): MediaEnvelope | null {
 ...
 */
function findMediaByHash(mediaRecords: MediaEnvelope[] | null | undefined, hash: string, mediaType: MediaType): MediaEnvelope | null {
 ...
 */
export function getMediaUrl(mediaRecords: MediaEnvelope[] | null | undefined, mediaId: string): string | null {
 ...
 *
 * @param mutations - { create, put, remove } from useMutations('media')
 * @param mediaRecords - current records from useQuery('media')
 * @param data - Blob or base64 data URL
...
export function extractMediaType(reference: string): string | null {
  if (!isMediaReference(reference)) return null
