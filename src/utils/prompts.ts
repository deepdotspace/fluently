...
/** Media generation toggles used by the bulk-generation prompt builders. */
interface MediaOptions {
  generateImages?: boolean
  generateAudio?: boolean
}

/** A field reference (only its id is read) used by the HTML layout generator. */
interface PromptFieldRef {
  id: string
}
...
function replaceTemplate(template: string, vars: Record<string, string | null | undefined>): string {
  let result = template;
...
export const PROMPTS = {
  ...
  CLASSIFICATION: (userPrompt: string, targetLang?: string, nativeLang?: string): string =>
  ...
  // Text generation
  TEXT_GENERATION: (refinedPrompt: string, targetLang?: string, nativeLang?: string): string =>
  ...
  // Image generation
  IMAGE_GENERATION: (refinedPrompt: string, targetLang: string = 'Spanish'): string =>
  ...
  // Audio generation
  AUDIO_GENERATION: (refinedPrompt: string, targetLang?: string, nativeLang?: string): string =>
  ...
  BULK_GENERATION_SYSTEM: (): string => PROMPTS_JSON.BULK_GENERATION_SYSTEM,

  // Card type specific instructions
  CARD_TYPE_INSTRUCTIONS: {
    CLOZE: (): string => PROMPTS_JSON.CARD_TYPE_CLOZE,
    BASIC_REVERSED: (): string => PROMPTS_JSON.CARD_TYPE_BASIC_REVERSED,
    BASIC: (): string => PROMPTS_JSON.CARD_TYPE_BASIC
  },

  // Auto generate prompt (from user prompt)
  BULK_GENERATION_AUTO: (
    targetCount: number,
    allowedTypes: string[],
    userPrompt: string,
...
[
  ...
  HTML_LAYOUT_GENERATOR: (userPrompt: string, frontFields: PromptFieldRef[], backFields: PromptFieldRef[]): string =>
  ...
  // AI Chat prompts
  AI_CHAT_INITIAL_SUGGESTIONS: (front: string, back: string, targetLang: string = 'Spanish', nativeLang: string = 'English'): string =>
    replaceTemplate(PROMPTS_JSON.AI_CHAT_INITIAL_SUGGESTIONS, { front, back, targetLang, nativeLang }),

  AI_CHAT_FOLLOW_UP_SUGGESTIONS: (lastResponse: string, targetLang: string = 'Spanish', nativeLang: string = 'English'): string =>
  ...
  AI_CHAT_RESPONSE: (front: string, back: string, messageText: string, targetLang: string = 'Spanish', nativeLang: string = 'English'): string =>
    replaceTemplate(PROMPTS_JSON.AI_CHAT_RESPONSE, { front, back, messageText, targetLang, nativeLang }),

  AI_CHAT_SIMPLIFY: (front: string): string =>
    replaceTemplate(PROMPTS_JSON.AI_CHAT_SIMPLIFY, { front }),

  AI_CHAT_EXAMPLE: (front: string, back: string): string =>
  ...
  MODIFY_TEMPLATE: (currentHtml: string, userPrompt: string): string => `You are an expert HTML/CSS designer for educational flashcards.
Current HTML:
${currentHtml}
...
- Return ONLY the modified HTML string.`,
  ...
  getAudioVoiceForLanguage: (targetLang: string): string => {
